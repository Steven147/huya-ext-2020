  ...
  try {
    if (
      ...
      const getCliVersion = () => {
        ...
        const cliPkgJson = fs.readJsonSync(path.resolve(projectAbsPath, 'node_modules/@hyext/cli/package.json'))
...
async function main() {
  const jsonContent = fs.readJsonSync(path.resolve(__dirname, '../package.json'))
  const program = new commander.Command(jsonContent.name)
...
function installBuilder(val, calllback, failcallback) {
  ...
  if (proc0.status === 0 && proc1.status === 0) {
    ...
    const pkgContent = fs.readJsonSync(pkgJsonPath)
